...
/*add by wangwei begin*/
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths password:(NSString *)password {
	BOOL success = NO;
	SSZipArchive *zipArchive = [[SSZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
		for (NSString *srcpath in paths) {
			[zipArchive writeFile:srcpath zipFile:path password:password];
		}
		success = [zipArchive close];
	}
...
/*add by wangwei begin*/
- (void)zipInfo:(zip_fileinfo*)zipInfo withFileDate:(NSString*)file
{
...
/*add by wangwei begin*/
- (BOOL)writeFile:(NSString *)path zipFile:(NSString *)zipFile password:(NSString *)password
{
    if ([password length] == 0) {
        [self writeFile:path];
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:zipFile]) {
        [self zipInfo:&zipInfo withFileDate:zipFile];
    } else {
	...
	ret = zipWriteInFileInZip( _zip, (const void*)[data bytes], (unsigned int)[data length]);
	if( ret!=Z_OK ) 
		return NO;
